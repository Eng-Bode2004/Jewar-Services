...
class ChiefProfileController {
  ...
  async updateOrder(req:Request, res:Response) {
    try {
      const result = await ChiefProfileServices.updateOrder(req.params.id, req.body);
      res.status(200).json(result);
    } catch (error:unknown) {
      res.status(400).json({
        status: "error",
  ...
  async updateDeliveryStep(req:Request, res:Response) {
    try {
      const { driver_id, step } = req.body;
      const result = await ChiefProfileServices.updateDeliveryStep(req.params.id, driver_id, step);
      res.status(200).json(result);
    } catch (error:unknown) {
      res.status(400).json({
        status: "error",
